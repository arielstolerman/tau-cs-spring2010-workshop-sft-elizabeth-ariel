...
function[L,coeffs,sizes]=sft_dp_rec(isLogged,G,tau,func,numOfIterations,delta_t,fInfNorm,fEuclideanNorm,deltaCoeff,randSetsCoeff,numOfRec)

% hold the size of each result
sizes = zeros(1,numOfRec);

[L,coeffs]=sft_dp(isLogged,G,tau,func,numOfIterations,delta_t,fInfNorm,fEuclideanNorm,deltaCoeff,randSetsCoeff);
sizes(1) = size(L,1);
% run numOfRec - 1 more times
for i=2:numOfRec
    [tmpL,tmpCoeffs]=sft_dp(isLogged,G,tau,func,numOfIterations,delta_t,fInfNorm,fEuclideanNorm,deltaCoeff,randSetsCoeff);
